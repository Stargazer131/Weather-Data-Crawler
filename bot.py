...
async def alert_handler(context: ContextTypes.DEFAULT_TYPE):
    data = get_next_hour_prediction(latest_data)

    answer = (
        f"The weather in next hour will be: {data['condition']} {icon_bank['weather'][data['icon']]}\n"
        f"Temperature: {data['temperature']:.1f}°C {icon_bank['weather']['temperature']}\n"
        f"Rain Probability: {data['rain_prob']:.2f}% {icon_bank['weather']['rain_prob']}\n"
        f"Thunderstorm Probability: {data['thunder_prob']:.2f}% {icon_bank['weather']['thunder_prob']}\n"
        f"Fog Probability: {data['fog_prob']:.2f}% {icon_bank['weather']['fog_prob']}\n"
        f"Please be careful!"
    )
...
async def alert_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(jobs) != 0:
        text = 'You already subscribed to alert update'
    else:
        context.job_queue.run_repeating(alert_handler, interval=60*60, first=2,
                                        chat_id=chat_id, name=f'alert_{chat_id}')
        text = 'You will be alerted for uncomfortable/hazardous weather every hour'
...
async def stop_alert_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(jobs) != 0:
        ...
        job.enabled = False
        job.schedule_removal()
        text = 'Stop receiving alert update!'
    else:
        text = "You haven't subscribed to alert update!"

    await context.bot.send_message(chat_id=chat_id, text=text)


def get_next_hour_prediction(input_data: tuple):
    timestamp, temp, humid, press, wind = input_data
    inputs = [temp, humid, press, wind,
              timestamp.month, timestamp.day, timestamp.hour]
    inputs = np.array(inputs).reshape(1, -1)
    ...
    # condition and probability
    temp_inputs = [predict_temperature, predict_humidity, predict_pressure, predict_wind_speed,
                   timestamp.month, timestamp.day, timestamp.hour+hour]
    temp_inputs = np.array(temp_inputs).reshape(1, -1)

    condition = inverse_transform_condition(classifier.predict(temp_inputs)[0]).lower()
    fog_prob = classifier.predict_proba(temp_inputs)[0][2] * 100
    rain_prob = classifier.predict_proba(temp_inputs)[0][3] * 100
    thunder_prob = classifier.predict_proba(temp_inputs)[0][4] * 100
    data['rain_prob'] = rain_prob
    data['thunder_prob'] = thunder_prob
    data['fog_prob'] = fog_prob
    time_of_day = day_or_night(timestamp.hour, hour)
    data['condition'] = condition
    if condition == 'fair':
        data['icon'] = time_of_day
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler("stop_alert", stop_alert_command))
